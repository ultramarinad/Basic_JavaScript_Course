...
var BARRIER_SPEED = 6000;   //!!!!!! Временной промежуток от препятствия к препятствию
...
function startGame() {
    if (!gameIsRunning) {
        ...
        barrier_timer = setInterval(barrierPlus, BARRIER_SPEED); // !!! Постройка барьера
    }
}
...
function barrierPlus() {
    var barCreated = false;

    while (!barCreated) {
        var bar_x = Math.floor(Math.random() * FIELD_SIZE_X); // Случайно по оси X
        var bar_y = Math.floor(Math.random() * FIELD_SIZE_Y); // Случайно по оси Y

        var bar_partition = document.getElementsByClassName('cell-' + bar_y + '-' + bar_x)[0];
        var bar_partition_clas = bar_partition.getAttribute('class').split(' ');

        if (!bar_partition_clas.includes('snake-unit') && !bar_partition_clas.includes('food-unit')) {
            bar_partition.classList.add('problem-unit')
            barCreated = true;
        }
    }
}
